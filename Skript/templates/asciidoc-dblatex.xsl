...
figure,table,equation,example
  ...
  <xsl:template match="processing-instruction('asciidoc-hr')">
   ...
   <!-- Überschreibe das Template für informalequation -->
  <xsl:template match="informalequation" mode="tex">
    <xsl:text>\begin{equation*}</xsl:text>
    <xsl:apply-templates select="*" mode="math"/>
    <xsl:text>\end{equation*}</xsl:text>
  </xsl:template>
  
    <!-- Anpassung für informalequation mit alt oder mathphrase -->
  <xsl:template match="informalequation" mode="tex">
    <xsl:choose>
      <xsl:when test="alt | mathphrase">
        <xsl:text>\[</xsl:text>
        <xsl:value-of select="normalize-space(alt | mathphrase)"/>
        <xsl:text>\]</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <!-- Standardverhalten (z.B. MathML) -->
